...
import { Engine3D as  RacingEngine} from '../game/engine3d';
...
export const RaceGame3D = forwardRef<RaceGameHandle, RaceGameProps>(function RaceGame(props, ref) {
  ...
  return (
    <div ref={stageRef} className={`game-stage ${phase === 'intro' ? 'is-intro' : ''} ${settings.scanlines ? 'has-scanlines' : ''} ${snapshot.boosting && phase === 'racing' ? 'is-boosting' : ''} ${expanded ? 'is-expanded' : ''}`} tabIndex={-1} aria-label="Nightshift 89 3D racing game">
      <canvas ref={canvasRef} className="game-canvas" role="img" aria-label="3D racing circuit. Use the arrow keys or WASD to drive, Shift to boost, and Escape to pause." />
